...
async function request<T>(
  ...
  if (!response.ok) {
    ...
    const firstFieldError = problem?.errors
      ...
      : undefined
    throw new ApiError(
      firstFieldError ?? problem?.detail ?? `Request failed (${response.status})`,
      response.status,
      problem?.errors
    )
  }
...
export function myListings(token: string, page = 0, size = 20): Promise<Page<Listing>> {
  return request<Page<Listing>>(`/api/listings/mine?page=${page}&size=${size}`, { token })
}
